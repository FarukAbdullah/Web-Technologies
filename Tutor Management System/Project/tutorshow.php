...
if(isset($_SESSION["valid"]) && $_SESSION["valid"]=="yes"){
	if(isset($_COOKIE["email"])){
		...
		$jsonData= getJSONFromDB("select email from login where type='Tutor' and status='active'");
		$data=json_decode($jsonData);

	    if(sizeof($data)>0){
		foreach($data as $v){?>
	    <a style="text-decoration: none" href="atprofile.php?email=<?php echo $v->email;?>"><?php echo $v->email;?></a><br/>
	<?php
	}
	}
else
{
	echo "<h3> No valid Tutor Found</h3>";
}
