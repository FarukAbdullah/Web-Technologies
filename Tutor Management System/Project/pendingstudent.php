...
<tr>
<td>
<h2>Pending List of Students</h2><hr>
...
if(isset($_SESSION["valid"]) && $_SESSION["valid"]=="yes"){
	if(isset($_COOKIE["email"])){
		...
		$jsonData= getJSONFromDB("select email from login where type='Student' and status='inactive'");
		$data=json_decode($jsonData);

        if(sizeof($data)>0){
		foreach($data as $v){?>
	    <a style="text-decoration: none" href="svalid.php?email=<?php echo $v->email;?>"><?php echo $v->email;?></a><br/>
	<?php
	}
	}
else
{
	echo "<h3> No Pending Student Found</h3>";
}
